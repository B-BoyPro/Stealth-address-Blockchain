...
def generate_stealth_meta_address():
    ...
    # Convert to hex 
    spending_private_hex = spending_private_key.to_string().hex()
    viewing_private_hex = viewing_private_key.to_string().hex()

    # Concatenate public keys to form stealth meta-address
    stealth_meta_address = f"st:eth:0x{spending_pub_hex}{viewing_pub_hex}"
    return stealth_meta_address, spending_private_hex, viewing_private_hex
...
def generate_stealth_address(sma):
    ...
    s_bytes = int(s.x()).to_bytes(32, byteorder='big') + int(s.y()).to_bytes(32, byteorder='big')
