...
from utils_stealth_address import point_to_eth_addr
...
def computeStealthAddress(ephemeral_pub_point, viewing_priv, spending_pub, view_tag_announcement):
    ...
    # Convert to SigningKey type 
    viewing_priv = SigningKey.from_string(bytes.fromhex(viewing_priv),curve = SECP256k1)
...
def computeStealthKey(ephemeral_pub_point, viewing_priv, spending_priv):
    ...
    # Convert to SigningKey type 
    viewing_priv = SigningKey(bytes.fromhex(viewing_priv),curve = SECP256k1)
    spending_priv = SigningKey(bytes.fromhex(spending_priv),curve = SECP256k1)
